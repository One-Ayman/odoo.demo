...
class TestFleetSalesFieldOps(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        # A cash journal with its own account spelled out explicitly --
        # letting Odoo auto-resolve an outstanding account for a
        # freshly-created journal isn't reliable in a bare test database.
        cash_account = cls.env["account.account"].create(
            {
                "name": "Test Field Cash Account",
                "code": "599001",
        ...
        # The outstanding account for an unreconciled payment lives on the
        # journal's payment method line, not the journal itself -- a fresh
        # journal's auto-generated "Manual" line has no account set yet.
    ...
    def test_payment_button_available_before_any_sale(self):
        ...
        visit = self._make_visit()
        self.salesman.cash_journal_id = self.cash_journal
        visit.action_check_in()
